...
export const SettingsView: React.FC<SettingsProps> = ({
  ...
  const { isAuthenticated, isGuest, user, signOut } = useAuth();
  ...
  const [customPcts, setCustomPcts] = useState<Record<number, number[]>>(
    profile.customPercentages || WEEK_MULTIPLIERS
  );
  const [customReps, setCustomReps] = useState<Record<number, number[]>>(
    profile.customReps || WEEK_REPS
  );
  const [editAssistance, setEditAssistance] = useState<
    Record<LiftType, string[]>
  >(profile.customAssistance || DEFAULT_ASSISTANCE);
  const [activeAssistanceLift, setActiveAssistanceLift] = useState<LiftType>(
    LiftType.Squat
  );
  const [showExercisePicker, setShowExercisePicker] = useState(false);
  const [editWarmups, setEditWarmups] = useState<WarmupSetSetting[]>(
    profile.warmupSettings || WARMUP_SETS
  );
  const [editLiftOrder, setEditLiftOrder] = useState<LiftType[]>(
    profile.liftOrder || Object.values(LiftType)
  );

  const isKg = profile.unit === "kg";
  const defaultInv = isKg
    ? DEFAULT_PLATE_INVENTORY_KG
    : DEFAULT_PLATE_INVENTORY_LBS;
  const [inventory, setInventory] = useState<Record<number, number>>(
    profile.plateInventory || defaultInv
  );
  ...
  const handleUnitChange = (unit: "lbs" | "kg") =>
    onUpdateProfile({ ...profile, unit, plateInventory: undefined });
  const toggleSound = () =>
  ...
  const handleTimerChange = (
    ...
    onUpdateProfile({
      ...profile,
      timerSettings: { ...profile.timerSettings, [type]: val },
    });
  };
  ...
  const removeAssistanceExercise = (lift: LiftType, exName: string) => {
    setEditAssistance({
      ...editAssistance,
      [lift]: editAssistance[lift].filter((e) => e !== exName),
    });
  };
  ...
  const addAssistanceExercise = (exName: string) => {
    setEditAssistance({
      ...editAssistance,
      [activeAssistanceLift]: [
        ...(editAssistance[activeAssistanceLift] || []),
        exName,
      ],
    });
  ...
  const handleWarmupChange = (
    ...
    if (field === "percentage")
      newWarmups[idx].percentage = parseFloat(val) / 100;
  ...
  return (
    <div className="p-4 space-y-6 animate-in slide-in-from-right duration-300">
      ...
      <div className="bg-card p-6 rounded-xl border border-slate-800">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center gap-3">
            ...
            <div>
              <h4 className="text-lg font-semibold text-white">Cloud Backup</h4>
        ...
        {isGuest ? (
          <div className="space-y-3">
            <p className="text-sm text-slate-300">
              Create an account to enable cloud sync and access your workouts
              from any device.
            </p>
            <Button fullWidth onClick={() => setShowAuthModal(true)}>
              Enable Cloud Sync
            </Button>
          </div>
        ) : (
          <div className="space-y-3">
            ...
            <p className="text-xs text-slate-500 text-center">
              Note: Local data will remain on this device
            </p>
          </div>
        )}
      </div>
      ...
      <ProgramSection
        ...
        onManageExercises={() =>
          onChangeView && onChangeView(AppView.ExerciseManager)
        }
      />
      ...
      <div className="bg-card p-4 rounded-xl border border-slate-800 space-y-6">
        ...
        <div className="grid grid-cols-2 gap-4">
          <div
            onClick={toggleSound}
            className={`p-3 rounded-lg flex items-center justify-between border transition-colors cursor-pointer ${
          ...
          >
            <span className="text-sm">{t.settings_voice}</span>{" "}
            <Mic size={18} />
          </div>
        </div>
            ...
            {["main", "supplemental", "assistance"].map((type) => (
              <div key={type}>
                  ...
                  <span>
                    {
                      profile.timerSettings?.[
                        type as keyof typeof profile.timerSettings
                      ]
                    }
                    s
                  </span>
                ...
                <input
                  ...
                  onChange={(e) =>
                    handleTimerChange(type as any, parseInt(e.target.value))
                  }
      ...
      <div className="bg-card p-4 rounded-xl border border-slate-800">
        <label className="block text-sm font-medium text-slate-300 mb-2 flex items-center">
          ...
          {t.settings_coach_mode}
        </label>
        <div className="flex items-center justify-between bg-slate-900 p-3 rounded-lg border border-slate-800">
          <span className="text-xs text-slate-400 max-w-[70%]">
            Enable tools for managing multiple clients and tracking their
            progress.
          </span>
          <button
            onClick={toggleCoachMode}
            className={`relative inline-flex h-6 w-11 items-center rounded-full transition-colors ${
              profile.isCoach ? "bg-blue-600" : "bg-slate-700"
            }`}
          >
            <span
              className={`inline-block h-4 w-4 transform rounded-full bg-white transition-transform ${
                profile.isCoach ? "translate-x-6" : "translate-x-1"
              }`}
            />
          </button>
        </div>
      ...
      <button
        ...
        className="text-xs text-red-500 font-bold uppercase tracking-wider flex items-center"
      >
      ...
      <div className="text-center pb-2">
        <p className="text-[10px] text-slate-600">
          Titan 531 v1.2.1 • Production Build
        </p>
      </div>

      {/* MODALS */}
      <Modal
        isOpen={showInventoryEdit}
        onClose={() => setShowInventoryEdit(false)}
        title="Plate Inventory"
      >
        <div className="space-y-4">
          <p className="text-xs text-slate-400">
            Specify how many of each plate you have.
          </p>
              ...
              >
                ...
                <input
                  ...
                  onChange={(e) =>
                    setInventory({
                      ...inventory,
                      [parseFloat(w)]: parseInt(e.target.value),
                    })
                  }
          ...
          <Button fullWidth onClick={saveInventory}>
            Save Inventory
          </Button>
        </div>
      </Modal>
      ...
      <Modal
        isOpen={showProgramEdit}
        onClose={() => setShowProgramEdit(false)}
        title="Custom Program"
      >
        <div className="space-y-6">
          <div className="flex justify-between items-center">
            <p className="text-xs text-slate-400">
              Edit main lift percentages and reps.
            </p>
            <button
              onClick={() => {
              ...
              >
                ...
                {[0, 1, 2].map((setIdx) => (
                  <input
                    ...
                    onChange={(e) =>
                      handlePctChange(week, setIdx, e.target.value)
                    }
              ...
              >
                ...
                {[0, 1, 2].map((setIdx) => (
                  <input
                    ...
                    onChange={(e) =>
                      handleRepChange(week, setIdx, e.target.value)
                    }
                    className="w-full bg-slate-900 border border-slate-700 rounded p-1 text-center text-white"
                  />
                ))}
          ...
          <Button fullWidth onClick={saveCustomProgram}>
            Save Program Logic
          </Button>
        </div>
      </Modal>
      ...
      <Modal
        isOpen={showAssistanceEdit}
        onClose={() => setShowAssistanceEdit(false)}
        title="Assistance Editor"
      >
        <div className="space-y-6 min-h-[50vh]">
          <p className="text-xs text-slate-400">
            Assign specific assistance exercises.
          </p>
          <div className="flex bg-slate-900 p-1 rounded-lg border border-slate-800 overflow-x-auto">
            {Object.values(LiftType).map((lift) => (
              <button
                key={lift}
                onClick={() => setActiveAssistanceLift(lift)}
                className={`flex-1 py-2 px-1 rounded text-xs font-bold whitespace-nowrap transition-all ${
                  activeAssistanceLift === lift
                    ? "bg-slate-700 text-white shadow"
                    : "text-slate-500"
                }`}
              >
          ...
          <div>
            <div className="flex justify-between items-center mb-2">
              <h4 className="text-sm font-bold text-white">
                Exercises for {activeAssistanceLift}
              </h4>
              <button
            ...
            <div className="space-y-2">
              {(editAssistance[activeAssistanceLift] || []).map((exName, i) => (
                <div
                  key={i}
                  className="flex justify-between items-center bg-slate-900 p-3 rounded-lg border border-slate-800"
                >
                  <span className="text-sm text-white">{exName}</span>
                  <button
                    onClick={() =>
                      removeAssistanceExercise(activeAssistanceLift, exName)
                    }
          ...
          <Button fullWidth onClick={saveAssistance}>
            Save Assistance Config
          </Button>
        </div>
      </Modal>
      ...
      >
        <div className="space-y-2 h-[60vh] overflow-y-auto">
          {[...(profile.customExercises || []), ...Object.values(EXERCISE_DB)]
            .sort((a, b) => a.name.localeCompare(b.name))
            .map((ex) => (
              <button
                key={ex.name}
                onClick={() => addAssistanceExercise(ex.name)}
                className="w-full text-left p-3 bg-slate-900 hover:bg-slate-800 rounded-lg border border-slate-800 text-sm text-white"
              >
                {ex.name}{" "}
      ...
      <AuthModal
        ...
        onSuccess={() => {
          ...
          showToast("Account created! Cloud sync enabled.", "success");
        }}
      />
    </div>
  );
