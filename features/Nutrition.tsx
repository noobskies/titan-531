...
import { Camera, Plus, Loader, Trash2, Utensils, Target, Activity, Search } from 'lucide-react';
...
export const Nutrition: React.FC<NutritionProps> = ({ profile, onUpdateProfile }) => {
        ...
        if (e.target.files && e.target.files[0]) {
            ...
            reader.onload = async () => {
                ...
                try {
                    ...
                    if (result) {
                        addMeal(result, reader.result as string); // Pass full data URL for display
                    }
                } catch (err) {
    ...
    const calculateTargets = (goal: 'lose' | 'maintain' | 'gain') => {
        ...
        const tdee = weightKg * 24 * 1.4; // Rough estimate for active lifter
        
        let cals = tdee;
        if (goal === 'lose') cals -= 500;
        if (goal === 'gain') cals += 300;

        // High protein split (30P/35C/35F approx)
        const protein = Math.round(weightKg * 2.2); // 1g per lb
                     ...
                     <div className="flex gap-2">
                         <div className="relative flex-1">
                            <input 
                                ...
                                className="w-full bg-slate-900 border border-slate-800 rounded-xl py-3 pl-4 pr-10 text-sm text-white focus:outline-none focus:ring-2 focus:ring-blue-500"
                         ...
                         <button 
                            ...
                            className="bg-slate-800 border border-slate-700 rounded-xl w-12 flex items-center justify-center text-slate-400 hover:text-white hover:bg-slate-700"
                         >
                             <Camera size={20} />
                         </button>
                         <input 
