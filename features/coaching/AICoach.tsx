...
import { getCoachAdvice } from '../../services/api/geminiService';
import { Send, ExternalLink, WifiOff } from 'lucide-react';
  ...
  return (
    <div className="flex flex-col min-h-[calc(100vh-7rem)] p-4">
        ...
        <div className="mt-auto space-y-2">
            {isOffline && (
                <div className="flex items-center justify-center space-x-2 p-2 bg-slate-800/50 rounded-lg text-slate-400 text-xs">
                    <WifiOff size={14} />
                    <span>Coach is offline. Reconnect to chat.</span>
                </div>
            )}
            <div className="flex space-x-2 pt-2">
                <input 
                    className="flex-1 bg-slate-800 border border-slate-700 rounded-xl px-4 py-3 text-white focus:outline-none focus:border-theme focus:ring-1 focus:ring-theme placeholder-slate-500 disabled:opacity-50 disabled:cursor-not-allowed"
                    placeholder={isOffline ? "Offline" : "Ask a question..."}
                    value={input}
                ...
                <button 
                    onClick={handleSend} 
                    className="bg-theme p-3 rounded-xl text-white hover:bg-theme-secondary disabled:opacity-50 disabled:cursor-not-allowed shadow-lg shadow-black/20 transition-colors" 
