...
import { calculateInitialTMs } from '../services/workoutLogic';
...
export const Onboarding: React.FC<OnboardingProps> = ({ onComplete }) => {
  ...
  const renderStep1 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
      ...
      <div className="space-y-4">
        ...
        <div className="grid grid-cols-2 gap-4">
            ...
            <div>
                 ...
                 <div className="flex bg-slate-800 rounded-xl p-1 border border-slate-700">
                    <button onClick={() => setUnit('lbs')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${unit === 'lbs' ? 'bg-theme text-white shadow' : 'text-slate-400 hover:text-white'}`}>LBS</button>
                    <button onClick={() => setUnit('kg')} className={`flex-1 py-2 rounded-lg text-sm font-medium transition-all ${unit === 'kg' ? 'bg-theme text-white shadow' : 'text-slate-400 hover:text-white'}`}>KG</button>
                 </div>
            </div>
        </div>
        <div>
        ...
        <div>
            <label className="block text-sm font-medium text-slate-300 mb-2 flex items-center"><Palette size={14} className="mr-2" /> App Theme</label>
            <div className="flex justify-between gap-2">
                {(Object.keys(THEME_COLORS) as ThemeColor[]).map(color => (
                    <button
                        key={color}
                        onClick={() => setThemeColor(color)}
                        className={`w-full h-10 rounded-lg border-2 transition-all ${themeColor === color ? 'border-white scale-110 shadow-lg' : 'border-transparent hover:border-slate-600'}`}
      ...
      <Button fullWidth size="lg" onClick={() => setStep(2)} disabled={!name || !weight}>Next: Strength</Button>
    </div>
  );
    ...
    <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
      <div className="text-center">
       ...
       <div className="grid grid-cols-2 gap-3 mb-4">
            <button onClick={() => setExperience('New')} className={`p-3 rounded-xl border text-center transition-all ${experience === 'New' ? 'bg-theme border-theme text-white' : 'bg-slate-800 border-slate-700 text-slate-400'}`}>
                <div className="font-bold text-sm">New</div>
                <div className="text-[10px] opacity-80 mt-0.5">Guide me</div>
            </button>
            <button onClick={() => setExperience('Experienced')} className={`p-3 rounded-xl border text-center transition-all ${experience === 'Experienced' ? 'bg-theme border-theme text-white' : 'bg-slate-800 border-slate-700 text-slate-400'}`}>
        ...
        {Object.values(LiftType).map(lift => (
            <div key={lift} className="flex items-center justify-between bg-slate-800 p-3 rounded-xl border border-slate-700">
                ...
                <div className="flex items-center space-x-2">
                    <input type="number" value={maxes[lift]} onChange={e => handleMaxChange(lift, e.target.value)} className="w-24 bg-slate-900 border border-slate-600 rounded-lg px-3 py-2 text-right text-white font-mono focus:border-blue-500 focus:outline-none" />
                    <span className="text-xs text-slate-500 font-bold w-6">{unit}</span>
                </div>
            </div>
        ))}
      </div>
      <Button fullWidth size="lg" onClick={() => setStep(3)}>Next: Schedule</Button>
    </div>
  );
  ...
  const renderStep3 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
        <div className="text-center">
            <div className="bg-theme w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-blue-900/50 transition-colors duration-300"><Calendar size={32} className="text-white" /></div>
        ...
        <div className="grid grid-cols-7 gap-2">
            {daysOfWeek.map((day, idx) => {
                ...
                return (
                    <button key={day} onClick={() => toggleDay(idx)} className={`aspect-square rounded-xl flex flex-col items-center justify-center border transition-all ${isSelected ? 'bg-theme border-theme text-white shadow-lg' : 'bg-slate-800 border-slate-700 text-slate-400 hover:bg-slate-700'}`}>
                        <span className="text-xs font-bold">{day.charAt(0)}</span>
                    </button>
                );
            })}
        </div>
        <div className="text-center text-xs text-slate-500">{trainingDays.length} days selected. We'll calculate your workout dates automatically.</div>
        <Button fullWidth size="lg" onClick={() => setStep(4)} disabled={trainingDays.length === 0}>Next: Choose Program</Button>
    </div>
  );
  ...
  const renderStep4 = () => (
    <div className="space-y-6 animate-in fade-in slide-in-from-right-8 duration-500">
      <div className="text-center">
        <div className="bg-theme w-16 h-16 rounded-2xl flex items-center justify-center mx-auto mb-4 shadow-lg shadow-blue-900/50 transition-colors duration-300"><Trophy size={32} className="text-white" /></div>
      ...
      <div className="space-y-3">
        {(Object.keys(PROGRAMS) as ProgramType[]).map(key => (
            <button key={key} onClick={() => setProgram(key)} className={`w-full text-left p-4 rounded-xl border transition-all ${program === key ? 'bg-theme-soft border-theme ring-1 ring-theme' : 'bg-slate-800 border-slate-700 hover:border-slate-600'}`}>
                ...
                <div className="inline-block bg-slate-900 text-[10px] px-2 py-1 rounded text-slate-500">Supp: {PROGRAMS[key].supplemental}</div>
            </button>
        ))}
      </div>
      <Button fullWidth size="lg" onClick={finish}>Start Training</Button>
    </div>
  );
  ...
  return (
    <div className="min-h-screen bg-darker flex flex-col justify-center p-6 relative overflow-hidden">
        ...
        <div className="mt-8 flex justify-center space-x-2 z-10">
            {[1, 2, 3, 4].map(i => (<div key={i} className={`h-1.5 rounded-full transition-all duration-300 ${i === step ? 'w-8 bg-theme' : 'w-2 bg-slate-700'}`} />))}
        </div>
    </div>
  );
};
