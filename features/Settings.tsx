...
import { ChevronLeft, Download, Upload, Trash2, AlertTriangle, Plus, ArrowUp, ArrowDown, Users, Volume2, VolumeX, Bell, Mic, Clock } from 'lucide-react';
...
import { AppearanceSection, ScheduleSection, ProgramSection } from './settings/SettingsSections';
...
export const SettingsView: React.FC<SettingsProps> = ({
  ...
  return (
    <div className="p-4 space-y-6 animate-in slide-in-from-right duration-300 pb-32">
      ...
      <Modal isOpen={showWarmupEdit} onClose={() => setShowWarmupEdit(false)} title="Warmup Config">
          <div className="space-y-4">
              <p className="text-xs text-slate-400">Customize your warmup progression.</p>
              <div className="space-y-2">
                  {editWarmups.map((set, i) => (
                      <div key={i} className="flex items-center gap-2 bg-slate-900 p-2 rounded-lg border border-slate-800">
                          <div className="flex items-center space-x-1 flex-1">
                             <input type="number" value={Math.round(set.percentage * 100)} onChange={e => handleWarmupChange(i, 'percentage', e.target.value)} className="w-12 bg-slate-800 border border-slate-700 rounded px-1 text-right text-white" /><span className="text-slate-500">%</span>
                          </div>
                          <span className="text-slate-600">x</span>
                          <div className="flex items-center space-x-1 flex-1">
                             <input type="number" value={set.reps} onChange={e => handleWarmupChange(i, 'reps', e.target.value)} className="w-12 bg-slate-800 border border-slate-700 rounded px-1 text-right text-white" /><span className="text-slate-500">reps</span>
                          </div>
                          <button onClick={() => setEditWarmups(editWarmups.filter((_, idx) => idx !== i))} className="text-red-500 p-1"><Trash2 size={16} /></button>
                      </div>
                  ))}
              </div>
              <button onClick={() => setEditWarmups([...editWarmups, { percentage: 0.5, reps: 5 }])} className="w-full py-2 border border-dashed border-slate-700 rounded-lg text-slate-500 hover:text-white text-xs flex items-center justify-center"><Plus size={14} className="mr-1" /> Add Set</button>
              <Button fullWidth onClick={saveWarmups}>Save Warmups</Button>
          </div>
       </Modal>

       <Modal isOpen={showLiftOrderEdit} onClose={() => setShowLiftOrderEdit(false)} title="Reorder Lifts">
           <div className="space-y-4">
               <p className="text-xs text-slate-400">Drag and drop order.</p>
               <div className="space-y-2">
                   {editLiftOrder.map((lift, i) => (
                       <div key={lift} className="flex items-center justify-between bg-slate-900 p-3 rounded-lg border border-slate-800">
                           <div className="flex items-center space-x-3">
                               <div className="w-6 h-6 rounded-full bg-slate-800 flex items-center justify-center text-xs font-bold text-slate-500">{i + 1}</div>
                               <span className="text-white font-medium">{lift}</span>
                           </div>
                           <div className="flex space-x-1">
                               <button onClick={() => moveLift(i, -1)} disabled={i === 0} className="p-1.5 hover:bg-slate-700 rounded text-slate-400 disabled:opacity-30"><ArrowUp size={16} /></button>
                               <button onClick={() => moveLift(i, 1)} disabled={i === editLiftOrder.length - 1} className="p-1.5 hover:bg-slate-700 rounded text-slate-400 disabled:opacity-30"><ArrowDown size={16} /></button>
                           </div>
                       </div>
                   ))}
               </div>
               <Button fullWidth onClick={saveLiftOrder}>Save Order</Button>
           </div>
       </Modal>
