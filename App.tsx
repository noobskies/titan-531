...
const ToolsView = React.lazy(() =>
  import("./features/tools/Tools").then((m) => ({ default: m.ToolsView }))
);
...
const Dashboard = React.lazy(() =>
  import("./features/shared/Dashboard").then((m) => ({ default: m.Dashboard }))
);
const HistoryView = React.lazy(() =>
  import("./features/history/History").then((m) => ({ default: m.HistoryView }))
);
...
const AICoachView = React.lazy(() =>
  import("./features/coaching/AICoach").then((m) => ({
    default: m.AICoachView,
  }))
);
const CoachDashboard = React.lazy(() =>
  import("./features/coaching/CoachDashboard").then((m) => ({
    default: m.CoachDashboard,
  }))
);
const ExerciseManager = React.lazy(() =>
  import("./features/shared/ExerciseManager").then((m) => ({
    default: m.ExerciseManager,
  }))
);
const Nutrition = React.lazy(() =>
  import("./features/shared/Nutrition").then((m) => ({ default: m.Nutrition }))
);
const Conditioning = React.lazy(() =>
  import("./features/shared/Conditioning").then((m) => ({
    default: m.Conditioning,
  }))
);
...
const AppContent: React.FC = () => {
  ...
  const getCompletedLiftsForWeek = (): LiftType[] => {
    return ctrl.activeHistory
      .filter(
        (s) =>
          s.cycle === ctrl.activeProfile.currentCycle &&
          s.week === ctrl.activeProfile.currentWeek &&
          s.type !== "Conditioning"
      )
  ...
  const unlockedAchObj = ACHIEVEMENTS.find(
    (a) => a.id === ctrl.newUnlockedAchievement
  );
  ...
  return (
    <Layout
      ...
      onExitClientView={() => {
        ctrl.setViewingProfileId("root");
        ctrl.setView(AppView.CoachDashboard);
      }}
    ...
    >
      ...
      >
        {ctrl.view === AppView.Workout && ctrl.activeSession && (
          <ActiveWorkout
            ...
            personalRecord={
              ctrl.activeProfile.oneRepMaxes[
                ctrl.activeSession.lift as LiftType
              ] || 0
            }
        ...
        {ctrl.view === AppView.Conditioning && (
          <Conditioning
            onSave={ctrl.saveConditioning}
            onCancel={() => ctrl.setView(AppView.Dashboard)}
            unit={ctrl.activeProfile.unit}
          />
        )}
        ...
        {ctrl.view === AppView.Dashboard && (
          <Dashboard
            ...
            onFinishWeek={() =>
              ctrl.activeProfile.currentWeek < 4 &&
              ctrl.saveProfile({
                ...ctrl.activeProfile,
                currentWeek: (ctrl.activeProfile.currentWeek + 1) as
                  | 1
                  | 2
                  | 3
                  | 4,
        ...
        {ctrl.view === AppView.History && (
          <HistoryView
            history={ctrl.activeHistory}
            onDeleteSession={ctrl.deleteWorkout}
            onUpdateSession={ctrl.updateHistorySession}
          />
        )}

        {ctrl.view === AppView.Nutrition && (
          <Nutrition
            profile={ctrl.activeProfile}
            onUpdateProfile={ctrl.saveProfile}
          />
        )}
        {ctrl.view === AppView.AICoach && (
          <AICoachView
            profile={ctrl.activeProfile}
            history={ctrl.activeHistory}
          />
        )}
        {ctrl.view === AppView.Profile && (
          <ProfileView
            profile={ctrl.activeProfile}
            history={ctrl.activeHistory}
            onUpdateProfile={ctrl.saveProfile}
        ...
        {ctrl.view === AppView.CoachDashboard && (
          <CoachDashboard
            ...
            onAddClient={(c) =>
              ctrl.updateRootProfile({
                ...ctrl.rootProfile,
                clients: [...(ctrl.rootProfile.clients || []), c],
              })
            }
            onSelectClient={(id) => {
              ctrl.setViewingProfileId(id);
              ctrl.setView(AppView.Dashboard);
            }}
        ...
        {ctrl.view === AppView.ExerciseManager && (
          <ExerciseManager
            profile={ctrl.activeProfile}
            onUpdateProfile={ctrl.saveProfile}
            onBack={() => ctrl.setView(AppView.Settings)}
          />
        )}
      </Suspense>

      <CycleTransition
        isOpen={ctrl.showCycleTransition}
        profile={ctrl.activeProfile}
        history={ctrl.activeHistory}
        onConfirm={ctrl.confirmCycleTransition}
        onCancel={() => ctrl.setShowCycleTransition(false)}
      />

      <Modal
        isOpen={!!ctrl.newUnlockedAchievement}
        onClose={() => ctrl.setNewUnlockedAchievement(null)}
        title=""
      >
        <div className="text-center p-6 space-y-4">
          ...
          <h3 className="text-2xl font-bold text-white uppercase tracking-wide bg-gradient-to-r from-yellow-500 to-orange-500 bg-clip-text text-transparent">
          ...
          <button
            onClick={() => ctrl.setNewUnlockedAchievement(null)}
            className="bg-slate-800 px-6 py-2 rounded-full text-white font-bold hover:bg-slate-700"
          >
            Awesome!
          </button>
        </div>
      ...
      <Modal
        isOpen={ctrl.showPremiumWall}
        onClose={() => ctrl.setShowPremiumWall(false)}
      ...
      >
        <div className="text-center p-4 space-y-4">
          ...
          <h3 className="text-xl font-bold text-white">
            Unlock {PROGRAMS[ctrl.activeProfile.selectedProgram]?.name}
          </h3>
          <p className="text-slate-400 text-sm">
            Upgrade to access FSL, Monolith, Advanced Analytics, and more.
          </p>
          <button
            onClick={() => {
              ctrl.saveProfile({ ...ctrl.activeProfile, isPremium: true });
              ctrl.setShowPremiumWall(false);
              showToast("Premium Unlocked (Demo)", "success");
            }}
          ...
          <button
            onClick={() => ctrl.setShowPremiumWall(false)}
