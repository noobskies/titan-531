...
function getAI(): GoogleGenAI {
  if (!ai) {
    ...
    if (!apiKey) {
      throw new Error(
        "Gemini API key not configured. Please add VITE_GEMINI_API_KEY to your .env.local file or disable AI features."
      );
    }
...
export const getCoachAdvice = async (
  ...
  try {
    ...
    return {
      text: "I'm having trouble connecting to the coaching server. Please check your connection and try again.",
    };
  }
};

export const generateWorkoutTip = async (
  cycle: number,
  week: number,
  lift: string
): Promise<string> => {
...
export const generateExerciseIllustration = async (
  exerciseName: string
): Promise<string | null> => {
  try {
    const response = await retry<GenerateContentResponse>(() =>
  ...
  try {
    const response = await retry<GenerateContentResponse>(() =>
      getAI().models.generateContent({
        model: "gemini-2.5-flash",
        contents:
          "Find top rated hardcore gyms, powerlifting gyms, or well-equipped strength training facilities near me. Provide a brief 1 sentence summary of their equipment for a powerlifter.",
        config: {
          tools: [{ googleMaps: {} }],
          // @ts-ignore
          toolConfig: {
    ...
    const mapPlaces = chunks
      .filter(
        (chunk: any) =>
          chunk.web?.uri?.includes("google.com/maps") || (chunk as any).maps
      )
...
export const analyzeNutrition = async (
  ...
  try {
    ...
    if (typeof input === "string") {
      parts.push({
        text: `Analyze the nutritional content of this food description: "${input}".`,
      });
    } else {
      parts.push({
        inlineData: {
    ...
    parts.push({
      text: "Provide the name of the meal and estimated macros (calories, protein, carbs, fats). Return ONLY raw JSON.",
    });
