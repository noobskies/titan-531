...
export const getCoachAdvice = async (
  ...
  try {
    ...
    return { text: "I'm having trouble connecting to the coaching server. Please try again." };
  }
};
...
export const findGymsNearby = async (lat: number, lng: number): Promise<{ text: string; places: any[] }> => {
  try {
    const response = await ai.models.generateContent({
      ...
      config: {
        tools: [{ googleMaps: {} }],
        // @ts-ignore - Types might be missing strict definition for Maps toolConfig in current SDK version
        toolConfig: {
          retrievalConfig: {
            latLng: {
              latitude: lat,
    ...
    const mapPlaces = chunks
      ...
      .map((chunk: any) => chunk.maps || chunk.web); // Adapting to potential response structure
